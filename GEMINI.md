...
- **2026-01-05**:
    ...
    6. **动态更新文件名**: 如果使用默认命名，系统会根据实际搜索进度动态缩减文件名（仅包含已搜职位），并自动清理旧名称文件，确保文件名准确且文件夹整洁。
- **2026-01-06**:
    1. **UI 优化**: 在终端输入阶段，隐藏目标职位输入框中由空默认值产生的 `()` 括号，使界面更清爽。
- **2026-01-08**:
    1. **文案调整**: 将用户输入提示从“请输入目标职位”改为“请输入关键词”，以更准确反映搜索逻辑。
    2. **早停优化**: 修复重复候选人计入早停计数器的问题。现在只有真正不符合条件的简历（登录时间、离职时间、AI不匹配、公司不匹配）才会计入早停，重复候选人会被跳过但不影响早停机制。
- **2026-01-09**:
    1. **Docx 保存重试机制**: `save_resume_as_docx` 函数新增失败自动重试功能（默认最多 3 次），增强保存稳定性。
    2. **保存失败处理**: 若 docx 最终保存失败，该候选人将被跳过，不会记录到 Excel 表格中，也不会计入合格简历数量，确保数据与文件的一致性。
- **2026-01-16**:
    1. **交互逻辑优化**: 去掉了“是否使用建议提纲? (Y/n)”的选择步骤。
    2. **提纲输入强制化**: 现在改为强制要求用户手动输入访谈提纲，不再提供默认提纲。
    3. **验证顺序优化**: 调整简历验证顺序，将 AI 判断移至最后一步，先进行所有字段匹配检查（登录时间、离职时间、公司名称、查重），大幅节省 AI API 调用成本。
